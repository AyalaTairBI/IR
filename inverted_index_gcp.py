...
class MultiFileReader:
    ...
    def __init__(self):
        self._open_files = {}
        self.client = storage.Client()
        self.bucket = self.client.bucket('assaignment316093632')

    def read(self, locs, n_bytes, base_dir):
        b = []
        for f_name, offset in locs:
            if f_name not in self._open_files:
                blob = self.bucket.get_blob(f'{base_dir}/{f_name}')
...
class InvertedIndex:  
    ...
    def posting_lists_iter(self, base_dir, query):
        ...
        with closing(MultiFileReader()) as reader:
            for w in query:
                ...
                b = reader.read(locs, self.df[w] * TUPLE_SIZE, base_dir)
                posting_list = []
